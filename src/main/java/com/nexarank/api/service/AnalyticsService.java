...
/**
 * NR-36: shared analytics aggregation, extracted out of AnalyticsController so
 * both the dashboard endpoints and the monthly summary email (which runs in a
 * @Scheduled context with no TenantContext/JWT) can build the same overview/
 * search-health/facet-usage/rules-performance data without duplicating the
 * aggregation logic. Callers resolve tenantId/projectId/window themselves —
 * this service has no notion of "the current request."
 */
@Service
public class AnalyticsService {
    ...
    /** Resolved reporting window — either startDate/endDate (inclusive, whole days) or the last N days from now. */
    public record Window(Instant since, Instant until) {}

    public Window resolveWindow(int days, String startDate, String endDate) {
    ...
    /** Calendar-month window for the given year/month (1-12), e.g. the previous full calendar month. */
    public Window monthWindow(int year, int month) {
        LocalDate start = LocalDate.of(year, month, 1);
        return new Window(start.atStartOfDay(ZoneOffset.UTC).toInstant(),
                start.plusMonths(1).atStartOfDay(ZoneOffset.UTC).toInstant());
    }

    public void addWindowMeta(Map<String, Object> response, Window window, String projectId) {
    ...
    public Map<String, Object> buildOverview(String tenantId, String projectId, Window window, int days) {
        ...
        Map<String, com.nexarank.api.model.MerchRule> ruleByZeroResultQuery = new HashMap<>();
    ...
    public List<Map<String, Object>> buildTrends(String tenantId, String projectId, int days, String startDate, String endDate) {
    ...
    public Map<String, Object> buildSearchHealth(String tenantId, String projectId, Window window, int days) {
        Instant since = window.since();
        Instant until = window.until();
    ...
    public Map<String, Object> buildFacetUsage(String tenantId, String projectId, Window window, int days) {
        Instant since = window.since();
        Instant until = window.until();
    ...
    public Map<String, Object> buildRulesPerformance(String tenantId, String projectId, Window window, int days) {
        Instant since = window.since();
        Instant until = window.until();
